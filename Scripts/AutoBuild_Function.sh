...
GET_TARGET_INFO() {
	...
	if [[ "${x86_Test}" == "x86_64" ]];then
		...
		if [[ "$(cat .config)" =~ "CONFIG_GRUB_EFI_IMAGES=y" ]];then
			INCLUDE_EFI_IMAGES=true
		else
			INCLUDE_EFI_IMAGES=false
		fi
	else
...
Diy_Part3_Base() {
	...
	[ -f ./tmp/Default_Firmware ] && rm -f ./tmp/Default_Firmware
	touch ./tmp/Default_Firmware
	case "${TARGET_PROFILE}" in
	x86_64)
		[[ "${INCLUDE_EFI_IMAGES}" == true ]] && echo "openwrt-x86-64-generic-squashfs-combined-efi.img" >> ./tmp/Default_Firmware
		echo "openwrt-x86-64-generic-squashfs-combined.img" >> ./tmp/Default_Firmware
		Firmware_sfx=".img"
	;;
	*)
		echo "openwrt-${TARGET_BOARD}-${TARGET_SUBTARGET}-${TARGET_PROFILE}-squashfs" >> ./tmp/Default_Firmware
		Firmware_sfx="-sysupgrade.bin"
	;;
	esac
	AutoBuild_Firmware="AutoBuild-${TARGET_PROFILE}-${Openwrt_Version}"
	Mkdir bin/Firmware
	cat ./tmp/Default_Firmware | while read Default_Firmware
	do
		Firmware_Path="bin/targets/${TARGET_BOARD}/${TARGET_SUBTARGET}"
		if [ -f "${Firmware_Path}/${Default_Firmware}" ];then
			_MD5=$(md5sum ${Firmware_Path}/${Default_Firmware} | cut -d ' ' -f1)
			_SHA256=$(sha256sum ${Firmware_Path}/${Default_Firmware} | cut -d ' ' -f1)
			if [ "${INCLUDE_EFI_IMAGES}" == true ];then
				AutoBuild_Detail="AutoBuild-${TARGET_PROFILE}-${Openwrt_Version}-efi.detail"
				echo -e "\nMD5:${_MD5}\nSHA256:${_SHA256}" > bin/Firmware/"${AutoBuild_Detail}"
				mv ${Firmware_Path}/${Default_Firmware}/${Default_Firmware} bin/Firmware/${AutoBuild_Firmware}-efi${Firmware_sfx}
			else
				echo -e "\nMD5:${_MD5}\nSHA256:${_SHA256}" > bin/Firmware/"${AutoBuild_Detail}"
				AutoBuild_Detail="AutoBuild-${TARGET_PROFILE}-${Openwrt_Version}.detail"
				mv ${Firmware_Path}/${Default_Firmware} bin/Firmware/${AutoBuild_Firmware}${Firmware_sfx}
			fi
		fi
	unset Default_Firmware _MD5 _SHA256
	done	
}
